...
WHERE c1.customerId < c2.customerId  // Evita duplicati
  AND toInteger(u1.tx_count) >= 5
  AND toInteger(u2.tx_count) >= 5
...
WITH c1, c2, shared_terminals, avg(toInteger(tx1.satisfaction_rating)) AS c1_avg_rating
MATCH (c2)-[:MADE_TRANSACTION]->(tx2:Transaction)
WITH c1, c2, shared_terminals, c1_avg_rating, avg(toInteger(tx2.satisfaction_rating)) AS c2_avg_rating
